...
def home():
    return(
        f"<center><h2>Welcome to the Hawii Climate Analysis Local API!</h2></center>"
...
def precip():
    #return the previous years year's precipitation as a json
    #Calculate the date on year from the laste date in data set.
...
def temperatures():
    ...
    #Calculate the date one year from the last date in the date set.
    previousYear = dt.date(2017,8,23) - dt.timedelta(days=365)
    
    #Perfrom a query to retrieve the temperatures form the most active station from the past year.
